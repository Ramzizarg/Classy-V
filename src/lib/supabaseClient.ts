/**
 * Supabase-compatible query builder backed by Neon (via `/api/neon/table`).
 * Lets the back office use a familiar `.from(table).select()/insert()/update()` API
 * without a real Supabase project — everything runs on the existing Neon database.
 */
...
class SelectBuilder {
  ...
  async execute() {
    ...
    if (this.shouldSingle)
      return { data: rows[0] ?? null, error: rows[0] ? null : { message: "No rows" }, count: r.count ?? null };
...
class MutBuilder {
  ...
  async execute() {
    return callNeon({ action: this.action, table: this.table, values: this.values, filters: this.filters });
  }
}
  ...
  from(table: string) {
    return {
      ...
      insert: (values: Record<string, unknown> | Record<string, unknown>[]) =>
        callNeon({ action: "insert", table, values }),
      upsert: (values: Record<string, unknown>, opts?: { onConflict?: string }) =>
        callNeon({ action: "upsert", table, values, onConflict: opts?.onConflict }),
