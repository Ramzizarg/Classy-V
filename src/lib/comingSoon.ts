...
const STORAGE_KEY = "classyv-coming-soon-settings";
...
function mergeWithDefaults(partial: Partial<ComingSoonSettings> | null): ComingSoonSettings {
  ...
  return {
    ...
    heroImageUrl:
      typeof partial.heroImageUrl === "string" ? partial.heroImageUrl : defaultComingSoonSettings.heroImageUrl,
...
/** Get coming soon settings (server-side only). Cached per request. */
...
/** Get coming soon settings from DB, then localStorage fallback, then defaults. */
