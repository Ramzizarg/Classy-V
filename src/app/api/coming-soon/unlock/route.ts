...
function getPepper() {
  return process.env.COMING_SOON_PASSWORD_PEPPER || process.env.COMING_SOON_COOKIE_SECRET || "dev-pepper";
}
...
function buildAccessCookieResponse(next: string, endAtIso: string | null): NextResponse {
  const exp =
    endAtIso && !Number.isNaN(Date.parse(endAtIso))
      ? Math.floor(Date.parse(endAtIso) / 1000)
...
export async function POST(req: Request) {
  ...
  try {
    const { rows } = await neonQuery(
      "SELECT enabled, require_password, password_hash, end_at FROM coming_soon_settings WHERE id = $1 LIMIT 1",
      ["default"]
    );
    const row = rows[0];
