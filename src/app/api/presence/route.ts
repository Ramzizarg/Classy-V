...
function isBot(ua: string): boolean {
  return /bot|crawl|spider|slurp|facebookexternalhit|bytespider|gptbot|claudebot|headlesschrome/i.test(ua);
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (contentType.includes("application/json")) {
      const body = (await req.json().catch(() => null)) as { visitorId?: string; path?: string } | null;
