...
function normalizeJsonColumnValue(value: unknown): unknown {
  ...
  if (typeof value === "string") {
    ...
    try {
      ...
      throw new Error(
        `Invalid JSON for a json/jsonb column (excerpt): ${t.slice(0, 160)}${t.length > 160 ? "…" : ""}`
      );
    }
  }
  return value;
  ...
  try {
      ...
      return NextResponse.json(
        {
          data: null,
          error:
            "DATABASE_URL is missing. Add the Neon connection string to .env.local, then restart the dev server (npm run dev).",
        },
        { status: 503 }
      );
    }
    ...
    if (action === "select") {
      ...
      const order = body.orderBy
        ? ` ORDER BY ${safeIdent(String(body.orderBy))} ${body.ascending === false ? "DESC" : "ASC"}`
        : "";
    ...
    if (action === "insert") {
      ...
      for (const row of values) {
        ...
        const safeRow = table === "products" ? sanitizeProductRow(raw) : sanitizeJsonColumns(table, raw);
    ...
    if (action === "upsert") {
      ...
      const updateCols = keys
        ...
        .join(", ");
      const sql = `INSERT INTO ${tableIdent} (${cols}) VALUES (${placeholders}) ON CONFLICT (${safeIdent(
        conflict
