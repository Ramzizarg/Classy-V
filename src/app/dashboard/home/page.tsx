...
const CS_LAST_PASSWORD_KEY = "classyv_cs_last_password";
...
export default function DashboardHomePage() {
  ...
  useEffect(() => {
    try {
      const v = localStorage.getItem(CS_LAST_PASSWORD_KEY) || "";
  ...
  const saveComingSoon = async () => {
    ...
    try {
      ...
      if (candidateNewPassword) {
        ...
        try {
          localStorage.setItem(CS_LAST_PASSWORD_KEY, candidateNewPassword);
        } catch {}
      }
  ...
  const selectProduct = (index: number, productId: string) => {
    ...
    if (!p) return;
    const priceStr =
      p.discount_price != null ? formatPrice(p.discount_price) : formatPrice(p.price);
    setContent((prev) => {
      const products = [...prev.products];
  ...
  const tabs = [
    ...
    { id: "bandeau" as const, label: "Banner", icon: PanelTop },
      ...
      {activeTab === "hero" && (
        <div className="space-y-6">
            ...
            <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    onChange={async (e) => {
                      ...
                      try {
                        ...
                        setHeroUploadError(err instanceof Error ? err.message : "Upload error");
                      } finally {
              ...
              {content.heroImageUrls.length > 0 ? (
                <div className="mt-3 space-y-2">
                  <p className="text-[10px] uppercase tracking-widest text-zinc-400">
                    Carousel order ({content.heroImageUrls.length} image{content.heroImageUrls.length > 1 ? "s" : ""})
                  </p>
              ...
              {selectedHeroPreviewUrl && (
                <div className="mt-3 flex justify-center">
                  ...
                  <div className="w-full">
                    <p className="text-[10px] uppercase tracking-widest text-zinc-400 mb-1.5">Desktop version</p>
                    <p className="text-[10px] text-zinc-500 mb-1.5">
                      Vertical position (top ↔ bottom) — image {selectedHeroPreviewIndex + 1}
                    </p>
                  ...
                  <div className="flex flex-col items-start justify-end lg:pt-0">
                    <p className="text-[10px] uppercase tracking-widest text-zinc-400 mb-1.5">Mobile version</p>
                    <p className="text-[10px] text-zinc-500 mb-1.5">
                      Selected image position (left ↔ right) — image {selectedHeroPreviewIndex + 1}
                    </p>
                    ...
                    <div className="relative w-[140px] sm:w-[160px] aspect-[9/16] max-h-[300px] rounded-xl overflow-hidden border-2 border-zinc-600 bg-black shadow-xl">
                      ...
                      <div className="absolute inset-0 flex items-center justify-between px-1.5 pointer-events-none">
                        ...
                        <span className="text-white/50 text-[6px] font-black font-mono tracking-[0.1em] uppercase">@CLASSYV</span>
                      </div>
              ...
              {content.heroImageUrls.length === 0 && (
                <div className="mt-2 flex items-center gap-2 text-zinc-500 text-xs">
                  ...
                  <span>No image. The site&apos;s default image will be shown.</span>
                </div>
              )}
            </div>
              ...
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-[11px] text-zinc-400 mt-1">
                    Disabled = images only. Enabled = shows only the filled fields below.
                  </p>
                </div>
                <button
                  type="button"
          ...
          <div className="rounded-lg border border-zinc-200 bg-zinc-950 p-4 sm:p-6 text-white">
            <p className="text-[10px] uppercase tracking-widest text-zinc-500 mb-3">Preview with texts</p>
            {selectedHeroPreviewUrl && (
            ...
            {content.heroShowOverlay ? (
              <div className="space-y-2">
                ...
                !content.heroButtonText.trim() &&
                !(content.heroSecondaryButtonText ?? "").trim() ? (
                  <p className="text-xs text-zinc-500">No text provided — the hero will show only images.</p>
                ) : null}
              </div>
            ) : (
              <p className="text-xs text-zinc-500">Texts disabled — the hero will show only images.</p>
            )}
        ...
        <div className="space-y-6">
          <div className="rounded-lg border border-zinc-700 bg-black p-4 sm:p-6 space-y-5">
            ...
            <p className="text-xs text-zinc-400">
              These phrases scroll in a loop at the top of the site. Edit the order or text, then save.
            </p>
                    ...
                    >
                      ...
                      {shopProducts.map((p) => (
                        <option key={p.id} value={p.id}>
                          {p.name} – {formatPrice(p.discount_price ?? p.price)}
                        </option>
                      ))}
                    </select>
                    {product.name && (
