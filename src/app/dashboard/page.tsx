...
async function getDashboardData() {
  ...
  try {
    const ordersRes = await neonQuery<{
      ...
      "SELECT id, reference, full_name, city, country, phone_number, email, total_price, status, created_at FROM shop_orders ORDER BY created_at DESC"
    );
    ...
    <div className="max-w-6xl mx-auto w-full min-w-0">
      ...
      {data.dataMissing ? (
        <div className="mb-6 rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-900">
          Neon is connected, but the back-office tables are not created yet (e.g. shop_orders, products). Run
          <code className="mx-1 rounded bg-amber-100 px-1">npm run shop:init</code>.
        </div>
      ) : null}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mb-6">
        <div className="rounded-xl border border-zinc-200 bg-gradient-to-br from-zinc-50 to-white px-4 py-4 flex flex-col min-h-[7.5rem]">
          ...
          <p className="mt-1 text-[11px] text-zinc-600">
            {data.pendingOrders} pending · {data.deliveredOrders} delivered
          </p>
          <p className="pt-1 text-[10px] text-zinc-500">Order tracking</p>
          <Link
        ...
        <div className="rounded-xl border border-zinc-200 bg-gradient-to-br from-zinc-50 to-white px-4 py-4 flex flex-col min-h-[7.5rem]">
          ...
          <p className="mt-1 text-[11px] text-zinc-600">
            {data.productsInStock} in stock
            {productsOos > 0 ? ` · ${productsOos} out of stock` : ""}
          </p>
          <p className="pt-1 text-[10px] text-zinc-500">Store catalog</p>
          <Link
        ...
        <div className="rounded-xl border border-black bg-gradient-to-br from-zinc-900 to-black px-4 py-4 flex flex-col min-h-[7.5rem] text-white">
          ...
          <p className="mt-1 text-[11px] text-white/75">Today {formatPrice(data.revenueToday)}</p>
          <p className="pt-1 text-[10px] text-white/50">Gross sales</p>
          <Link
            href="/dashboard/analytiques"
